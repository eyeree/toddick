...
var http = require( 'toddick/lib/http' );
...
toddick( 'Server', module,
  {
    ...
    INIT: function( portal, port ) {
      ...
      this.server = node_http.createServer( this.REQUEST.sync );
    ...
    SET_PROXY_ARG: function( args, arg_name, message_name, proxy ) {
      ...
      if( message_name ) {
        if( !proxy[ message_name ] ) {
          ...
          var def = args[ arg_name ];
          var msg = proxy[ message_name ];
          if( def.args ) {
            msg = msg.withArgs.apply( msg, def.args );
          }
          args[ arg_name ] = msg;
        }
      } else {
...
toddick( 'ProxyDefRequest', module,
  {
    ...
    INIT: function( url, MSG ) {
      ...
      this.client_supervisor.START( 'GET', url, MSG );
...
toddick( 'MessageSendRequest', module,
  {
    ...
    SET_MESSAGE_ARG: function( args, arg_name, msg ) {
      ...
      if( msg.getArgs ) {
        msg.proxy_def.args = msg.getArgs();
      }
    ...
    SEND_MESSAGE: function() {
      this.link( new http.JsonClient( 'POST', this.url, this.msg ) );
    }
