...
var toddick = function (export_name, export_module, handlers ) {
  ...
  var constructor = function() {
    ...
    internal_this.exit = function (reason) {
      ...
      internal_this.trace('exiting', {reason: reason, monitors: monitors, monitoring: monitoring});
    ...
    internal_this.createDispatcher = function (message_name, handler, inactive_handler) {
        ...
        if (is_active) {
          try {
            ...
            if(message_name === 'EXIT') {
              ...
              external_this.EXIT.sync(exception);
            }
          }
        } else {
          if(inactive_handler) {
      ...
      // ##### Property: is_toddick_dispatcher
      //
      // Exists and is true for all toddick dispatchers. This property is available as 
      // *dispatcher*.is_toddick_dispatcher on a message dispatch function.
      //
      //      var is_dispatcher = dispatcher.is_toddick_dispatcher;
      //
      dispatcher.is_toddick_dispatcher = true;
